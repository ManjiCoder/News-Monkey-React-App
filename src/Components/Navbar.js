    ...
    <div className="container-fluid">
      ...
      <div className="collapse navbar-collapse" id="navbarSupportedContent">
        ...
        <form className="d-flex mb-2 mb-lg-0" onSubmit={(event) => { event.preventDefault() }}>
          ...
          <Link className='text-decoration-none text-light' to={text}><button disabled={text.length === 0} className="btn btn-outline-success" type="submit" onClick={()=> props.getText(text)}>Search</button></Link>
        </form>
      </div>
    </div>
  </nav>
